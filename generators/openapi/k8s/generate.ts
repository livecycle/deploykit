...
${Object.entries(properties).map(([key, prop])=>
  ...
  ${formatTSPropName(key)}${prop.required || required.includes(key) ? "" : "?"}:${compileType(prop)};
  `
).join("")}
${additionalProperties && `[key:string]: ${compileType(additionalProperties)}` || ""}
}`
)
  }
  if (def.type === "string" ){
...
`import { ${[...new Set(names)].join(",")}} from "${relative(dirname(location), importLocation)}"`
    ...
    let typesContent = types.map(({name, def})=> {
      ...
      let gvk = def["x-kubernetes-group-version-kind"] as GroupVersionKind[] | null
      if (gvk && gvk.length > 0 && Object.keys(def.properties || {}).includes("apiVersion") ){
         let apiVersion = [gvk[0].group, gvk[0].version].filter(x=> x!= "").join("/")
         let funcDef = 
`
...
export function addStubModules(locations: string[]) : {location: string, content: string }[] {
  let tree = {}
  ...
  for (let current = nodesToVisit.shift(); current != null; current =nodesToVisit.shift() ){
